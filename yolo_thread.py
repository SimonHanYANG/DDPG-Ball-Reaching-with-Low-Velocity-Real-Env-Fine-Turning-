...
try:
    ...
    print("Warning: JPDAF tracker not available")
...
class TensorRTInference:
    ...
    def __init__(self, engine_path):
        ...
        try:
            ...
            logger.error(f"No active CUDA context: {e}")
            raise RuntimeError("Must initialize TensorRT in thread with active CUDA context")
...
class YOLODetectionThread(QThread):
    ...
    detection_result_signal = pyqtSignal(np.ndarray, list, list, list, dict)
    status_signal = pyqtSignal(str, str)
    fps_signal = pyqtSignal(float, float)
    ...
    def __init__(self, engine_path="weights/yolov8-weights/best.engine"):
        ...
        # CUDA context (will be created in run())
    ...
    def initialize_cuda(self):
        """Initialize CUDA context - 必须在线程的run()方法中调用"""
        try:
            # 初始化CUDA驱动
            cuda.init()
            ...
            logger.info(f"CUDA context created on device: {self.cuda_device.name()}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to initialize CUDA: {e}")
            self.status_signal.emit(f"CUDA initialization failed: {str(e)}", "error")
            return False
    
    def initialize_engine(self):
        """Initialize YOLO engine and tracker"""
        try:
    ...
    def run(self):
        ...
        try:
            ...
            self.status_signal.emit(f"Thread error: {str(e)}", "error")
            ...
            if self.cuda_context:
                try:
                    ...
                    logger.info("CUDA context destroyed")
                except Exception as e:
                    logger.error(f"Error destroying CUDA context: {e}")
